...
def rearrange_clusters(image_list):
    temp_pseudolabels = []
    temp_image_indexes = []
    for cluster, images in enumerate(image_list):
        temp_image_indexes.extend(images)
        temp_pseudolabels.extend([cluster] * len(images))

    temp_indexes_sorted = np.argsort(temp_image_indexes)  
    return np.asarray(temp_pseudolabels)[temp_indexes_sorted]
...
def preprocess_features(npdata, pca=128):
    ...
    mat = faiss.PCAMatrix(ndim, pca, eigen_power=-0.5) #remove hardcode
    mat.train(npdata)
